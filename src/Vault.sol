...
contract Vault is ERC20Upgradeable, AccessControl, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _mintSharesFor(address recipient, uint256 _amount, uint256 _pool) internal returns (uint256 shares) {
        if (totalSupply() == 0) {
            shares = _amount;
        } else {
